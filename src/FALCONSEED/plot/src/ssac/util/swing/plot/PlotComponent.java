 ...
 */
/*
 * @(#)PlotPane.java	2.1.0	2013/07/08
 ...
 * @since 2.1.0
 */
public class PlotComponent extends PlotBox
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	//------------------------------------------------------------
	// Override ptolemy.plot.PlotBox interfaces
	//------------------------------------------------------------

	/**
	 * 指定された <em>xpos</em> と <em>ypos</em> の位置で、指定したデータセットに対応する
	 * マークを描画する。データセットごとのマークスタイルに依存する。
	 * デフォルトのマークを使用する場合は <code>_marks</code> 変数に設定されている種類で描画する。
	 * <em>clip</em> が <tt>true</tt> の場合、描画エリアにクリップされる。
	 * <p>このメソッドは同期されない。
	 * なお、このメソッドは、イベントディスパッチスレッドから呼び出す必要がある。
	 * @param graphics	グラフィックオブジェクト
	 * @param dataset	データセットのインデックス
	 * @param xpos		<code>x</code> の位置を示す値
	 * @param ypos		<code>y</code> の位置を示す値
	 * @param clip		描画エリアにクリップする場合は <tt>true</tt>
	 */
	@Override
	protected void _drawPoint(Graphics graphics, int dataset, long xpos, long ypos, boolean clip) {
		...
		if (!fmt.marksUseDefault) {
			// 対象データセットがデフォルトマークを使用しない場合は、データセットごとのマークを使用する。
			marks = fmt.marks;
		}
		
		//_drawPoint(graphics, dataset, xpos, ypos, clip, marks);
		localDrawPoint(graphics, dataset, xpos, ypos, clip, marks);
	}
	...
	/**
	 * 指定された <em>xpos</em> と <em>ypos</em> の位置で、指定したデータセットに対応する
	 * マークを描画する。描画するマークは、<em>marks</em> に指定された種別に応じたマークとなる。
	 * <em>clip</em> が <tt>true</tt> の場合、描画エリアにクリップされる。
	 * <p>このメソッドは同期されない。
	 * なお、このメソッドは、イベントディスパッチスレッドから呼び出す必要がある。
	 * @param graphics	グラフィックオブジェクト
	 ...
	 * @param ypos		<code>y</code> の位置を示す値
	 * @param clip		描画エリアにクリップする場合は <tt>true</tt>
	 * @param marks		描画するマークの種類
	 */
	private void localDrawPoint(Graphics graphics, int dataset, long xpos, long ypos, boolean clip, final int marks) {
...
// TODO: modified	_drawPoint(graphics, dataset, xpos, ypos, true, 2 /*dots*/);
                    localDrawPoint(graphics, dataset, xpos, ypos, true, 2 /*dots*/);
    ...
    public synchronized void samplePlot() {
        ...
        Runnable sample = new RunnableExceptionCatcher(new Runnable() {
            public void run() {
                synchronized (PlotComponent.this) {
	...
	//------------------------------------------------------------
	// Original ptolemy.plot.Plot class implements
	//------------------------------------------------------------
    ///////////////////////////////////////////////////////////////////
    ////                         public methods                    ////
...
// TODO: modified		_drawPoint(graphics, dataset, prevxpos, prevypos, true, 2 /*dots*/);
                        localDrawPoint(graphics, dataset, prevxpos, prevypos, true, 2 /*dots*/);
                    } else {
...
// TODO: modified		_drawPoint(graphics, dataset, xpos, ypos, true, updatedMarks);
                        localDrawPoint(graphics, dataset, xpos, ypos, true, updatedMarks);
                        prevypos = ypos;
                        prevxpos = xpos;
                    }
                }
