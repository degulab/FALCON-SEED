 ...
 * @since 2.1.0
 */
public class PlotPane extends PlotBox
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
...
//	private boolean _firstInSet = true;
	...
	public PlotPane(PlotModel model) {
		super();
		setPlotModel(model);
	}
	...
	protected void updateChartSettings() {
		if (_plotImage != null && !_plotModel.isModifiedSettings() && _plotModel.isDataConstraintsUpdated()) {
			// 変更なし
			return;
		}
		...
		// 数値制約の更新
		_plotModel.updateDataConstraintsByRecords();
		...
		// xtick
		{
			PlotDataSeries series = _plotModel.getSeries(0);
			for (long ri = 0; ri < series.getValueCount(); ++ri) {
				double value = _plotModel.convertXDecimalToDouble(series.getXDecimalValue(ri));
				if (!Double.isNaN(value)) {
					addXTick("X-Value:" + String.valueOf(value), value);
				}
			}
		}
		...
		// ytick
		{
			PlotDataSeries series = _plotModel.getSeries(0);
			for (long ri = 0; ri < series.getValueCount(); ++ri) {
				double value = _plotModel.convertYDecimalToDouble(series.getYDecimalValue(ri));
				if (!Double.isNaN(value)) {
					addYTick("Y-Value:" + String.valueOf(value), value);
				}
			}
		}
	}
