 ...
 */
public class Union extends ssac.aadl.runtime.AADLModule
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	protected int aadlRun(String[] args) {
		if (args.length < 6) {
			System.err.println("'Union' module needs 6 arguments! : " + args.length);
			return (1);
		}
		...
		try {
			...
			println("Union...");
			if (tempFile3 != null) {
				lastResult = union(tempFile1, tempFile2, tempFile3, keepOrder, recComparator, firstSortResult1, firstSortResult2);
			} else {
				lastResult = union(tempFile1, tempFile2, outFile, keepOrder, recComparator, firstSortResult1, firstSortResult2);
			}
			println(".....done.");
	...
	//			それ以外の場合は null を返す。
	// AADL:function union(inFile1:String, inFile2:String, outFile:String, keepOrder:Boolean, recComparator:Object, 
	// 							sortResult1:DecimalList, sortResult2:DecimalList):DecimalList
	protected java.util.List<java.math.BigDecimal> union(String inFile1, String inFile2, String outFile,
														...
														java.util.List<java.math.BigDecimal> sortResult2)
	{
		...
		//@{
		// 最大フィールド数の計算
		//--- 重複しないレコードを出力するため、両テーブルで最大のフィールド数にあわせる
		int maxNumFields = Math.max(sortResult1.get(3).intValue(), sortResult2.get(3).intValue());
		//--- テーブル2の順序維持用番号のオフセット設定
		java.math.BigDecimal orderOffset = (keepOrder ? sortResult1.get(0) : null);
		...
		try {
			...
			// 和集合
			dataRecords = unionRecords(orderOffset, maxNumFields, sortResult1.get(2).longValue(), sortResult2.get(2).longValue(),
	...
	/**
	 * 重複のない和集合を出力する。
	 * @param orderOffset		テーブル2レコードの順序維持用番号のオフセット値、順序維持しない場合は <tt>null</tt>
	 ...
	 * @return	成功した場合は出力したレコード数、それ以外の場合は <tt>null</tt>
	 */
	protected java.math.BigDecimal unionRecords(java.math.BigDecimal orderOffset, int maxNumFields,
												...
												ssac.aadl.runtime.io.CsvFileWriter fout)
	{
		...
		for (; rec1 != null && rec2 != null; ) {
			...
			if (cmp > 0) {
				...
				if (orderOffset != null) {
					// 順序維持する
					if (!writeRecordModifyOrder(orderOffset, rec2, maxNumFields, fout)) {
						return null;	// 出力エラー
					}
				} else {
					// 順序維持しない
					if (!writeRecord(rec2, maxNumFields, fout)) {
						return null;	// 出力エラー
					}
				}
				//--- 同一の右レコードはスキップ
