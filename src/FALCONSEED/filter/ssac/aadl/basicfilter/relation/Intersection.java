 ...
 */
/*
 * @(#)Intersection.java	0.1.0	2013/08/08
 *     - created by Y.Ishizuka(PieCake.inc,)
 */
package ssac.aadl.basicfilter.relation;

/**
 * テーブルの積(Intersection)計算フィルタ。
 * 
 ...
 */
public class Intersection extends ssac.aadl.runtime.AADLModule
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	public static void main(String[] args) {
		Intersection module = new Intersection();
	...
	protected int aadlRun(String[] args) {
		if (args.length < 6) {
			System.err.println("'Intersection' module needs 6 arguments! : " + args.length);
			return (1);
		}
		...
		try {
			// ソート用テンポラリファイル作成
			tempFile1 = createTemporaryFile("inter1_", ".csv");
			tempFile2 = createTemporaryFile("inter2_", ".csv");
			if (keepOrder) {
				tempFile3 = createTemporaryFile("inter3_", ".csv");
			}
			...
			if (tempFile3 != null) {
				lastResult = intersection(tempFile1, tempFile2, tempFile3, keepOrder, recComparator, firstSortResult1, firstSortResult2);
			} else {
				lastResult = intersection(tempFile1, tempFile2, outFile, keepOrder, recComparator, firstSortResult1, firstSortResult2);
			}
			println(".....done.");
	...
	//			それ以外の場合は null を返す。
	// AADL:function intersection(inFile1:String, inFile2:String, outFile:String, keepOrder:Boolean, recComparator:Object, 
	// 							sortResult1:DecimalList, sortResult2:DecimalList):DecimalList
	protected java.util.List<java.math.BigDecimal> intersection(String inFile1, String inFile2, String outFile,
														...
														java.util.List<java.math.BigDecimal> sortResult2)
	{
		...
		try {
			...
			// 和集合
			dataRecords = intersectRecords(maxNumFields, sortResult1.get(2).longValue(), sortResult2.get(2).longValue(),
	 ...
	 * @return	成功した場合は出力したレコード数、それ以外の場合は <tt>null</tt>
	 */
	protected java.math.BigDecimal intersectRecords(int maxNumFields,
												...
												ssac.aadl.runtime.io.CsvFileWriter fout)
	{
		...
		// 残りレコードはどちらかのテーブルにしか存在しないので、出力しない
