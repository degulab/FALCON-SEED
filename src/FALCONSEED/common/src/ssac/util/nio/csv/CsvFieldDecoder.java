 ...
 */
/*
 * @(#)CsvFieldDecoder.java	1.17	2010/11/19
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)CsvFieldDecoder.java	1.16	2010/09/27
...
/**
 * 文字列をCSVフィールドに分割するクラス。
 * <p>このクラスは、デコード対象文字列を格納する文字列バッファの内容を順次読み取り、
 * CSVフィールドに分割する。文字列バッファの内容がCSVレコードとして不完全であっても、
...
public class CsvFieldDecoder
{
	...
	public CsvFieldDecoder(final StringBuilder stringBuffer) {
		this._stringBuffer = Validations.validNotNull(stringBuffer);
	 ...
	 */
	public int decode() {
		Validations.validState(_lastState != DS_RECORD_END, "Already reached record end.");
		
		if (isQuoteEscapeEnabled()) {
