 ...
 */
/*
 * @(#)CmdCsvFieldCellNode.java	3.3.0	2016/05/10
...
/**
 * <code>[Excel to CSV]</code> 変換定義の '#csv-field-cell' コマンド。
 * 
 * @version 3.3.0
 * @since 3.3.0
 */
public class CmdCsvFieldCellNode extends AbCommandNode
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	
	static public final String	CMD_NAME	= "#csv-field-cell";
	...
	public CmdCsvFieldCellNode(CellPosition cmdpos) {
		super(cmdpos);
	}
	...
	throws IOException, EtcException
	{
		// 値の出力
		//--- 進捗はカウントしない
		if (_fieldParam.getValue() instanceof CellIndex) {
			// 相対列位置
			CellIndex celidx = (CellIndex)_fieldParam.getValue();
			int rowIndex = absTargetRect.getFirstRowIndex() + celidx.getRowIndex();
			int colIndex = absTargetRect.getFirstColumnIndex() + celidx.getColumnIndex();
