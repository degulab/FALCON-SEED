 ...
 */
/*
 * @(#)LibVersionDialog.java	1.21	2012/08/21
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)LibVersionDialog.java	1.14	2009/12/09
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)LibVersionDialog.java	1.10	2009/01/21
 ...
 * このダイアログでは、AADLエディタのライブラリパスに格納されている
...
public class LibVersionDialog extends BasicDialog
{
	...
	public LibVersionDialog(Frame owner) {
		super(owner, EditorMessages.getInstance().LibVersionDlg_Title_Main, true);
	...
	protected void setupButtonPanel() {
		...
		this.btnApply.setText(EditorMessages.getInstance().LibVersionDlg_Title_CopyToClip);
	...
	/**
	 * AADLエディタが使用するライブラリのバージョン情報を更新する。
	 */
	protected void updateVersionInfo() {
		taInfo.removeAll();
		File baseLibPath = AADLEditor.getLibDirFile();
	...
	protected void setJarFileVersionInfo(File file) {
		...
		//--- パス
		appendPathParamString(EditorMessages.getInstance().LibVersionDlg_Title_Info_Path, file.getParent());
		//--- 最終更新日時
		appendTimeParamString(EditorMessages.getInstance().LibVersionDlg_Title_Info_LastMod, file.lastModified());
		...
		// マニフェスト情報の設定
		//--- Title
		appendParamString(EditorMessages.getInstance().LibVersionDlg_Title_Info_JarTitle, strTitle);
		//--- Version
		appendParamString(EditorMessages.getInstance().LibVersionDlg_Title_Info_JarVersion, strVersion);
