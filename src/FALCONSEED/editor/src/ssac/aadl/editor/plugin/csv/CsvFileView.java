 ...
 */
/*
 * @(#)CsvFileView.java	1.19	2012/02/23
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)CsvFileView.java	1.17	2011/02/04
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)CsvFileView.java	1.17	2010/11/19
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)CsvFileView.java	1.16	2010/09/27
...
import ssac.aadl.editor.plugin.macro.MacroMenuBar;
...
public class CsvFileView extends AbSpreadSheetView<CsvFileModel>
{
	...
	protected void setupComponentsAfterSetDocument() {
		...
		/*@@@ deleted by Y.Ishizuka : 2010.08.15 @@@
		// 最初に表示されたときのみ、表示領域のデータでカラム幅を調整
		this.addComponentListener(new ComponentListener(){
			public void componentHidden(ComponentEvent e) {}
			public void componentMoved(ComponentEvent e) {}
			public void componentResized(ComponentEvent e) {}
			public void componentShown(ComponentEvent e) {
				if (_shownTableFirstTime) {
					_shownTableFirstTime = false;
					//System.err.println("[Debug] CsvFileView adjust all column width by viewbox(" + String.valueOf(getDocument().getViewBox()) + ")");
					getTableComponent().adjustAllColumnsPreferredWidth();
				}
			}
		});
		//@@@ end of deleted : 2010.08.15 @@@*/
	}
	...
	public EditorMenuBar getDocumentMenuBar() {
		return super.getDocumentMenuBar();
	}
	...
	static protected class CsvFileViewTable extends SpreadSheetTable
	{
		...
		//@@@ modified by Y.Ishizuka : 2010.11.19 @@@
		@Override
		...
		/*--- old codes : 2010.11.19 ---
		@Override
		protected int getAdjustedColumnWidth(int columnIndex, TableColumn columnHeader) {
			int beginRowIndex, endRowIndex;
			TableModel model = getModel();
			if (model instanceof CsvFileModel) {
				// 表示領域のデータのみでカラム幅を調整
				...
				// 表示領域を持たないモデルの場合は、テーブル全体
				beginRowIndex = 0;
				endRowIndex   = getRowCount();
			}
			
			// 幅を計算
			return getAdjustedColumnWidth(beginRowIndex, endRowIndex, columnIndex, columnHeader, true);
		}
		/*--- end of old codes : 2010.11.19 ---*/
		//@@@ modified by Y.Ishizuka : 2010.11.19 @@@
	}
