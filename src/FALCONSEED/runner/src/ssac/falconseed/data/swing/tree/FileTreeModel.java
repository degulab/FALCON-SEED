/*
 * @(#)FileTreeModel.java	4.0.0	2021/08/23 : for Java11
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)FileTreeModel.java	1.10	2011/02/14
...
public class FileTreeModel extends AbstractTreeModel
{
	...
	/** クラス標準のファイルコンパレータ **/
	//static protected Comparator<FileTreeNode>	_defComparator;
	...
	//private Comparator<FileTreeNode>	_comparator;
	...
	public FileTreeModel(FileTreeNode rootnode) {
		this(rootnode, getDefaultFileFilter(), getDefaultFileTreeNodeComparator());
	}
	
	//public FileTreeModel(FileTreeNode rootnode, VirtualFileFilter filter, Comparator<FileTreeNode> comparator)
	//{
	//	super(rootnode);
	...
	//	}
	//}
	public FileTreeModel(FileTreeNode rootnode, VirtualFileFilter filter, Comparator<? super TreeNode> comparator)
	{
		super(rootnode);
		this._fileFilter = filter;
		this._comparator = comparator;
	 ...
	 * @throws IllegalArgumentException	指定されたパスが、このモデルのものではない場合
	 */
	public String formatFilePath(FileTreeNode node) {
	 ...
	 * @return	設定されている <code>Comparator&lt;FileTreeNode&gt;</code> オブジェクトを返す。
	 * 			設定されていない場合は <tt>null</tt> を返す。
	 */
	//public Comparator<FileTreeNode> getNodeComparator() {
	//	return _comparator;
	//}
	 ...
	 * @param c	新たに設定する <code>Comparator&lt;FileTreeNode&gt;</code> オブジェクト
	 */
	//public void setNodeComparaotr(Comparator<FileTreeNode> c) {
	//	if (_comparator != c) {
	//		_comparator = c;
	//		removeAllChildrenFromAvailableRootNode();
	//		reload();
	 ...
	 * @throws NullPointerException	引数が <tt>null</tt> の場合
	 */
	public void changeFileObject(FileTreeNode targetNode, VirtualFile newFile) {
		targetNode.setUserObject(targetNode.getUserObject().replaceFile(newFile));
		int numChildren = targetNode.getChildCount();
		for (int i = 0; i < numChildren; i++) {
			FileTreeNode child = (FileTreeNode)targetNode.getChildAt(i);
	 ...
	 */
	public void sortChildren(FileTreeNode parentNode) {
	 ...
	 */
	public void insertFileTreeNode(FileTreeNode parent, FileTreeNode newNode) {
	 ...
	 */
	public FileTreeNode createDirectory(FileTreeNode parent, String name)
	throws VirtualFileOperationException
	{
		...
		// ディレクトリノードの作成
		FileTreeNode newNode = createFileTreeNodeByFile(parent, fDir);
	...
	public void setRoot(TreeNode root) {
		if (!(root instanceof FileTreeNode))
			throw new IllegalArgumentException("The specified TreeNode is not instance of FileTreeNode.");
		super.setRoot(root);
	}
	...
	public void refresh() {
		if (_rootNode != null) {
			refreshNode((FileTreeNode)_rootNode);
		}
	}
	...
	public void refreshChildren() {
		if (_rootNode != null) {
			refreshChildren((FileTreeNode)_rootNode);
		}
	}
	 ...
	 */
	public void refreshNode(FileTreeNode targetNode) {
	...
	public void refreshChildren(FileTreeNode targetNode) {
		// ファイル構造により更新
		ITreeFileData file = targetNode.getUserObject();
		if (file.isDirectory()) {
			// ファイルリストを取得
			ITreeFileData[] childFileArray = file.listFiles(getFileFilter());
			if (childFileArray != null && childFileArray.length > 0) {
				// ディレクトリにファイルあり
				Set<ITreeFileData> files = new LinkedHashSet<ITreeFileData>(Arrays.asList(childFileArray));
				
				// ファイルの存在しない子ノード、ならびにファイルリストに存在しない子ノードを除去
				for (int i = targetNode.getChildCount() - 1; i >= 0; i--) {
					FileTreeNode child = (FileTreeNode)targetNode.getChildAt(i);
					ITreeFileData childFile = child.getUserObject();
					if (!childFile.exists()) {
						// 実体ファイルのないノード
						targetNode.remove(i);
				...
				FileTreeNode dummyNode = new FileTreeNode(file);
				for (ITreeFileData f : files) {
					dummyNode.setUserObject(f);
					int index = targetNode.binarySearch(dummyNode, getNodeComparator());
					if (index < 0) {
						// create new node
						FileTreeNode newNode = new FileTreeNode(f);
						insertNodeInto(newNode, targetNode, (Math.abs(index) - 1));
					} else {
						// already exists
						FileTreeNode existChild = (FileTreeNode)targetNode.getChildAt(index);
	...
	protected String formatFilePath(int beginIndex, TreePath treepath) {
		...
		if (beginIndex >= 0 && len > beginIndex) {
			for (int i = beginIndex; i < len; i++) {
				FileTreeNode node = (FileTreeNode)treepath.getPathComponent(i);
	...
	protected String formatFilePath(int beginIndex, TreeNode[] nodepath) {
		...
		if (beginIndex >= 0 && len > beginIndex) {
			for (int i = beginIndex; i < len; i++) {
				sb.append(Files.CommonSeparatorChar);
				sb.append(((FileTreeNode)nodepath[i]).getDisplayName());
			}
		} else {
			sb.append(Files.CommonSeparatorChar);
		}
	 ...
	 */
	protected TreePath getTreePathForFile(FileTreeNode ndSearchRoot, VirtualFile targetFile)
	{
		...
		FileTreeNode targetNode = ndSearchRoot;
	 ...
	 * @return	<code>Comparator&lt;FileTreeNode&gt;</code> オブジェクト
	 */
	//static protected Comparator<FileTreeNode> getDefaultFileTreeNodeComparator() {
	 ...
	 * @param file		抽象パス
	 * @return	<code>FileTreeNode</code> オブジェクト
	 */
	protected FileTreeNode createFileTreeNodeByFile(FileTreeNode parent, VirtualFile file) {
		return new FileTreeNode(new TreeFileData(file));
	}
	 ...
	 */
	protected int getInsertionIndex(FileTreeNode parent, FileTreeNode child) {
		if (parent == null)
			throw new IllegalArgumentException("parent argument is null.");
		if (child == null)
			throw new IllegalArgumentException("child argument is null.");
		
		//Comparator<? super FileTreeNode> c = getNodeComparator();
	...
	static public class DefaultFileTreeNodeComparator implements Comparator<TreeNode>
	{
		public int compare(TreeNode node1, TreeNode node2) {
			...
			if (node1 instanceof FileTreeNode) {
				FileTreeNode mn = (FileTreeNode)node1;
			...
			if (node2 instanceof FileTreeNode) {
				FileTreeNode mn = (FileTreeNode)node2;
	...
	/*** old source ***
	static public class DefaultFileTreeNodeComparator implements Comparator<FileTreeNode>
	{
