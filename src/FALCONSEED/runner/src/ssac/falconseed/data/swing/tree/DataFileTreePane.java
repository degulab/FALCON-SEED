 ...
 */
/*
 * @(#)DataFileTreePane.java	1.10	2011/02/14
 ...
 * @since 1.10
 */
public class DataFileTreePane extends JPanel
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
		...
		if (newValidator != null) {
			...
			if (path == null) {
				...
				newValidator.setAlreadyExistsFilenames(
						((FileTreeNode)path.getLastPathComponent()).getFileObject()
				);
			}
		}
	}
	 ...
	 * ツリーコンポーネントを生成する。
	 */
	protected DataFileTree createTreeComponent() {
		DataFileTree tree = new DataFileTree();
	 ...
	 */
	protected class NoDragDropTreeHandler extends DataFileTree.DefaultDataFileTreeHandler
	{
		@Override
