 ...
 */
/*
 * @(#)FindDialog.java	1.00	2010/12/20
...
public class FindDialog extends BasicDialog
{
	...
	public FindDialog(Frame owner, FindReplaceInterface handler) {
		super(owner, RunnerMessages.getInstance().FindDlg_Title_Main_WithReplace, false);
	...
	static public void showNotFoundMessage(Component parentComponent, String keyword) {
		...
		if (!Strings.isNullOrEmpty(keyword)) {
			ModuleRunner.showMessageBox(parentComponent, RunnerMessages.getInstance().FindDlg_Title_Main_FindOnly,
									"\"" + keyword + "\" " + CommonMessages.getInstance().msgSearchResultNotFound,
	...
	static public void showReplacedTokensMessage(Component parentComponent, int tokens) {
		// 一度に置換した文字列の個数を表示
		ModuleRunner.showMessageBox(parentComponent, RunnerMessages.getInstance().FindDlg_Title_Main_WithReplace,
								String.format(CommonMessages.getInstance().msgSearchReplacedTokens, tokens),
								JOptionPane.INFORMATION_MESSAGE);
	}
	...
	public void setFocus() {
		SwingUtilities.invokeLater(new Runnable(){
			public void run() {
				if (!fldKeyword.hasFocus()) {
					if (!fldKeyword.requestFocusInWindow()) {
						fldKeyword.requestFocus();
					}
				}
			}
		});
	...
	protected void updateFunctionButtons() {
		if (handler != null) {
			...
			lblReplace.setEnabled(handler.allowReplaceOperation());
			fldReplace.setEnabled(handler.allowReplaceOperation());
			btnReplaceNext.setEnabled(handler.allowReplaceOperation());
			btnReplaceAll.setEnabled(handler.allowReplaceOperation());
			lblReplace.setVisible(handler.allowReplaceOperation());
			fldReplace.setVisible(handler.allowReplaceOperation());
			btnReplaceNext.setVisible(handler.allowReplaceOperation());
			btnReplaceAll.setVisible(handler.allowReplaceOperation());
			if (handler.allowReplaceOperation()) {
				setTitle(RunnerMessages.getInstance().FindDlg_Title_Main_WithReplace);
			} else {
				setTitle(RunnerMessages.getInstance().FindDlg_Title_Main_FindOnly);
			}
		} else {
			btnFindPrev.setEnabled(false);
			btnFindNext.setEnabled(false);
			lblReplace.setEnabled(false);
			fldReplace.setEnabled(false);
			btnReplaceNext.setEnabled(false);
			btnReplaceAll.setEnabled(false);
			lblReplace.setVisible(true);
			fldReplace.setVisible(true);
			btnReplaceNext.setVisible(true);
...
//		// テスト
	...
	private JPanel createTextFieldPanel() {
		// items
		//--- find
		JLabel lblFind = new JLabel(RunnerMessages.getInstance().FindDlg_Title_FindText);
		this.fldKeyword = new KeywordTextField();
		//--- replace
		lblReplace = new JLabel(RunnerMessages.getInstance().FindDlg_Title_ReplaceText);
		this.fldReplace = new KeywordTextField();
		//--- options
		this.chkCaseInsensitive = new JCheckBox(RunnerMessages.getInstance().FindDlg_Title_Option_CaseInsensitive);
		//--- Panel
	...
	private JPanel createFunctionPanel() {
		// items
		this.btnReplaceNext = new JButton(RunnerMessages.getInstance().FindDlg_Button_ReplaceNext);
		this.btnReplaceAll = new JButton(RunnerMessages.getInstance().FindDlg_Button_ReplaceAll);
		this.btnFindPrev = new JButton(RunnerMessages.getInstance().FindDlg_Button_FindPrev);
		this.btnFindNext = new JButton(RunnerMessages.getInstance().FindDlg_Button_FindNext);
		//--- Panel
		JPanel panel = new JPanel(new GridBagLayout());
