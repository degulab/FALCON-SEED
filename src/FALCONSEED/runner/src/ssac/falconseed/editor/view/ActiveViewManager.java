 ...
 */
/*
 * @(#)ActiveViewManager.java	1.20	2012/03/23
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)ActiveViewManager.java	1.00	2010/12/20
...
public class ActiveViewManager
{
	...
	/**
	 * アクティブなコンポーネントが存在しない場合でも、最後にアクティブだったコンポーネントのボーダーをキープする。
	 * @since 1.20
	 */
	private boolean	_keepLastActivate = true;	// default : keep last activation
	...
	/**
	 * フォーカスを持つコンポーネントが存在しない場合でも、
	 * 直前にアクティブだったコンポーネントのボーダーを維持する場合に <tt>true</tt> を返す。
	 * @since 1.20
	 */
	public boolean isKeepLastActivation() {
		return _keepLastActivate;
	}

	/**
	 * フォーカスを持つコンポーネントが存在しない場合に、直前にアクティブだったコンポーネントの
	 * ボーダーを維持するかどうかを設定する。<em>toKeep</em> に <tt>true</tt> を指定した場合、
	 * 登録されているすべてのコンポーネントがフォーカスを失った場合でも、直前にアクティブだった
	 * コンポーネントのボーダーは変更されない。
	 * @param toKeep	維持する場合は <tt>true</tt>、維持しない場合は <tt>false</tt>
	 * @since 1.20
	 */
	public void setKeepLastActivation(boolean toKeep) {
		if (_keepLastActivate != toKeep) {
			_keepLastActivate = toKeep;
			if (!toKeep && _activated != null) {
				// 維持しない設定に変更されたとき、アクティブでないコンポーネントのボーダー変更
				if (!_activated.isFocusOwner()) {
					setActivatedComponent(null);
				}
			}
		}
	}
	...
	protected void onFocusOwnerChanged(PropertyChangeEvent pce) {
		...
		// フォーカスオーナーなし
		if (!_keepLastActivate) {
			// フォーカスオーナーが未登録の場合、アクティブなコンポーネント表示は行わない
			setActivatedComponent(null);
		}
		// _keepLastActivate=true なら、現在のアクティベーションを変更しない
	}
