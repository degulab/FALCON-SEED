 ...
 */
/*
 * @(#)AbComponentManager.java	1.22	2012/08/22
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)AbComponentManager.java	1.00	2010/12/20
...
public abstract class AbComponentManager implements IComponentManager
{
	 ...
	 * ファイル選択用のファイルフィルタ
	 */
	protected ExtensionFileFilter[] forOpenFilters;
	/**
	 * ファイル保存用のファイルフィルタ
	 */
	protected ExtensionFileFilter[] forSaveFilters;
	/**
	 * 最後に選択されたファイル
	 */
	...
	/**
	 * ファイル・オープン・ダイアログ用ファイルフィルタを設定する。
	 * 指定された引数が <tt>null</tt> もしくは要素が空の場合、要素が空の配列を設定する。
	 * @param filters	設定するフィルターの配列
	 */
	public void setOpenFileFilters(ExtensionFileFilter...filters) {
		if (filters != null && filters.length > 0) {
			ExtensionFileFilter[] aryFilters = new ExtensionFileFilter[filters.length];
			System.arraycopy(filters, 0, aryFilters, 0, filters.length);
			this.forOpenFilters = aryFilters;
		} else {
			this.forOpenFilters = EMPTY_FILTERS;
		}
	}
	
	/**
	 * ファイル・セーブ・ダイアログ用ファイルフィルタを設定する。
	 * 指定された引数が <tt>null</tt> もしくは要素が空の場合、要素が空の配列を設定する。
	 * @param filters	設定するフィルターの配列
	 */
	public void setSaveFileFilters(ExtensionFileFilter...filters) {
		if (filters != null && filters.length > 0) {
			ExtensionFileFilter[] aryFilters = new ExtensionFileFilter[filters.length];
			System.arraycopy(filters, 0, aryFilters, 0, filters.length);
			this.forSaveFilters = aryFilters;
		} else {
			this.forSaveFilters = EMPTY_FILTERS;
		}
	}
	...
	public boolean onSaveComponent(Component parentComponent, IEditorView targetView)
	{
		...
		try {
			...
			String errmsg = CommonMessages.getErrorMessage(
					CommonMessages.MessageID.ERR_FILE_READ, ex,
	...
	/**
	 * 保存先となるファイルを、ファイルダイアログで選択した結果を取得する。
	...
	protected File chooseSaveFile(Component parentComponent, String dialogTitle) {
		...
		do {
			...
			if (lastSelectedFile != null && !lastSelectedFile.isDirectory()) {
