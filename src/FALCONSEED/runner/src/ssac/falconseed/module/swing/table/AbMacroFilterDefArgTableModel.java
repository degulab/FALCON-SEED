 ...
 */
/*
 * @(#)AbMacroFilterDefArgTableModel.java	3.1.0	2014/05/18
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)AbMacroFilterDefArgTableModel.java	2.0.0	2012/10/28
...
/**
 * マクロフィルタの引数定義用データモデル。
 * このクラスは、<code>MacroFilterEditModel</code> の内部クラスとして
 * 実体が生成される。
 * 
 * @version 3.1.0	2014/05/18
 * @since 2.0.0
 */
public abstract class AbMacroFilterDefArgTableModel extends AbMExecDefArgTableModel implements IModuleArgConfigTableModel
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	static protected void rotate(MacroFilterDefArgList list, int a, int b, int shift) {
		int size = b - a; 
		int r = size - shift;
		int g = gcd(size, r); 
		for(int i = 0; i < g; i++) {
