 ...
 */
/*
 * @(#)GenericExpressionElementEditModel.java	3.2.1	2015/07/22
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)GenericExpressionElementEditModel.java	3.2.0	2015/06/28
...
/**
 * 汎用フィルタの計算式設定用テーブルにおける要素となるデータモデル。
 * 
 * @version 3.2.1
 * @since 3.2.0
 */
public class GenericExpressionElementEditModel extends SchemaExpressionData implements GenericSchemaElementValidation
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	
	static public final String	NAME_PREFIX	= SchemaExpressionData.TYPE_NAME;
	...
	public GenericExpressionElementEditModel() {
		super();
	}

	public GenericExpressionElementEditModel(String operator, SchemaElementValue leftValue, SchemaElementValue rightValue) {
		super(operator, leftValue, rightValue);
		refreshResultValueType();
	}
	
	public GenericExpressionElementEditModel(final SchemaExpressionData src) {
		super(src);
	}
	...
	public boolean verify() {
		...
		if (lop instanceof SchemaLiteralValue) {
			...
			if (literal.getSpecifiedText() == null) {
				...
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_NoLeftLiteralValueType);
				return false;
			}
			if (literal.getSpecifiedText().isEmpty()) {
				// Empty literal value
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_EmptyLeftLiteralValue);
				return false;
			}
			if (!literal.isValidSpecifiedValue()) {
				// Failed to convert literal value to specified value type
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_InvalidLeftLiteralValue);
				return false;
			}
		}
		//--- right operand
		if (rop instanceof SchemaLiteralValue) {
			...
			if (literal.getSpecifiedText() == null) {
				// No value type selection
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_NoRightLiteralValueType);
				return false;
			}
			if (literal.getSpecifiedText().isEmpty()) {
				// Empty literal value
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_EmptyRightLiteralValue);
				return false;
			}
			if (!literal.isValidSpecifiedValue()) {
				// Failed to convert literal value to specified value type
				setError(RunnerMessages.getInstance().msgGenericFilterEdit_InvalidRightLiteralValue);
				return false;
			}
		}
