...
/**
 * 汎用フィルタの CSV 出力テーブルスキーマ編集用データモデル。
 * 
 * @version 3.2.1
 * @since 3.2.0
 */
public class OutputCsvTableSchemaEditModel extends SchemaOutputCsvDataTable implements GenericSchemaTreeData, GenericSchemaElementValidation
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	 ...
	 */
	public OutputCsvTableSchemaEditModel() {
		super();
	}
	 ...
	 */
	public OutputCsvTableSchemaEditModel(int initialCapacity) {
	 ...
	 */
	public OutputCsvTableSchemaEditModel(boolean copyListElements, SchemaOutputCsvDataTable srcModel) {
		super(false, srcModel);	// ここでは要素はコピーしない
		if (copyListElements) {
			//--- deep copy
			for (SchemaOutputCsvDataField field : srcModel) {
				super.add(new OutputCsvFieldSchemaEditModel(field));
			}
		}
		if (srcModel instanceof OutputCsvTableSchemaEditModel) {
			OutputCsvTableSchemaEditModel srcEditModel = (OutputCsvTableSchemaEditModel)srcModel;
	...
	@Override
	public OutputCsvTableSchemaEditModel getData() {
		return this;
	}
