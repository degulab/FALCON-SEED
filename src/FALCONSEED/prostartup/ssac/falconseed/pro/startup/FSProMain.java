/*
 * @(#)FSProMain.java	4.0.0	2021/08/23
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)FSProMain.java	2014/10/20
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)FSProMain.java	2012/07/03
 *     - modified by Y.Ishizuka(PieCake.inc,)
 * @(#)FSProMain.java	2011/02/14
 *     - created by Y.Ishizuka(PieCake.inc,)
 */
package ssac.falconseed.pro.startup;
...
/**
 * FALCON-SEED 開発者向けランチャー起動専用アプリケーション
 * 
 * @version 4.0.0
 * @since 2.0.0
 */
public class FSProMain
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	static private final String ADVANCED_OPTION = "-advanced";
	...
	public static void main(String[] args) {
		FSProMain appmain = new FSProMain(args);
		if (appmain.isVerboseMode()) {
			System.out.println("<<< Start '" + APP_NAME + "-Pro' startup program >>>");
		}
		...
		if (result != 0) {
			...
			sb.append("Failed to start " + APP_NAME + "-Pro!");
			...
			JOptionPane.showMessageDialog(null, strerr, APP_NAME + "-Pro", JOptionPane.ERROR_MESSAGE);
			System.exit(result);
		}
		if (appmain.isVerboseMode()) {
			System.out.println("<<< '" + APP_NAME + "-Pro' startup program completed! >>>");
		}
	}
