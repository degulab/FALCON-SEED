 ...
 */
/*
 * @(#)IntegerRangeImpl.java	1.70	2011/06/29
...
/**
 * <tt>int</tt> で表現可能な数値範囲オブジェクト。
 * この数値範囲では、範囲先頭、範囲終端、数値間隔を保持し、
 * 範囲終端を含む数値範囲を表します。
 * <p>この数値範囲は、<tt>int</tt> で表現可能な数値範囲のため、
 * 範囲先頭、範囲終端、数値間隔が <tt>int</tt> で表現可能な値であり、
 * 要素の算出において <tt>int</tt> で演算可能な数値であることが保証されます。
 ...
 * @since 1.70
 */
public class IntegerRangeImpl implements NumberRangeImpl
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	public IntegerRangeImpl(int from, int to, int step) {
		if (!RangeUtil.exactlyIterableIntegerRange(from, to, step)) {
			step = 0;
		}
	 ...
	 * @return	<code>short</code> 範囲内の数値である場合は <code>Short.class</code>、そうでない場合は <code>Integer.class</code>
	 */
	...
	public boolean isIncludeValue(BigDecimal value) {
		...
		if (_step > 0) {
			// incremental
			if (_from <= intValue && intValue <= _to) {
				return true;
			} else {
				return false;
			}
			...
			// decremental
			if (_to <= intValue && intValue <= _from) {
				return true;
			} else {
				return false;
			}
	...
	public boolean containsValue(BigDecimal value) {
		...
		if (((value.longValue() - (long)_from) % (long)_step) == 0L) {
			// (value - _from) % _step == 0
			return true;
		} else {
			// (value - _from) % _step != 0
