...
public class MqttSessionImplTest
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	@Test
	public void testMqttSessionImpl() {
		try {
			new MqttSessionImpl(null);
		...
		session = new MqttSessionImpl(params);
		assertFalse(session.isConnected());
		assertEquals(session.getServerURI(), params.getServerURI());
		assertEquals(session.getClientID(), params.getClientID());
	}
	...
	public void testConnection() {
		MqttSessionImpl session = null;
		MqttSessionImpl session1 = null;
		MqttSessionImpl session2 = null;
		try {
			MqttConnectionParams params = new MqttConnectionParams();
			session = new MqttSessionImpl(params);
			assertFalse(session.isConnected());
			...
			session1 = new MqttSessionImpl(params1);
			session2 = new MqttSessionImpl(params2);
	...
	public void testSetEventHandler() {
		MqttSessionImpl session = new MqttSessionImpl(new MqttConnectionParams());
	...
	static class MessageChecker {
		...
		public byte[]  pubPayload;
		public int     pubQos;
		public boolean pubRetained;

		public boolean subArrived;
		public String  subTopic;
		public String  subMessage;
		public byte[]  subPayload;
		public MqPayload	subMqPayload;
		public int     subQos;
		public boolean subRetained;
		public boolean subDuplicated;
		
		public void clearPubMessage() {
			pubTopic    = null;
			pubMessage  = null;
			pubPayload  = null;
			pubQos      = 0;
		...
		public void setPubMessage(String topic, String message, int qos, boolean retained) {
			pubTopic    = topic;
			pubMessage  = message;
			pubPayload  = message.getBytes();
			pubQos      = qos;
			pubRetained = retained;
			clearSubMessage();
		}
		
		public void setSubMessage(MqttArrivedMessage message) throws MqttException
		{
			subArrived   = true;
			subTopic     = message.getTopic();
			subPayload   = message.getPayload().toArray();
			subMessage   = message.getPayload().toString();
			subMqPayload = message.getPayload();
			subQos       = message.getQos();
			subRetained  = message.isRetained();
			subDuplicated = message.isDuplicated();
		}
		
		public boolean waitMessageArrived(long timeout) {
			if (timeout > 0L) {
				long startTime = System.currentTimeMillis();
				long endTime = startTime;
				long passedTime = endTime - startTime;
				for (; passedTime >= 0L && passedTime < timeout; ) {
					try {
						Thread.sleep(100);
		...
		public void assertMessageArrived(long timeout) {
			assertTrue(waitMessageArrived(timeout));
		}
		
		public void assertNoMessageArrived(long timeout) {
			assertFalse(waitMessageArrived(timeout));
		}
		
		public void assertCompleted() {
			assertTrue(subArrived);
			assertEquals(pubTopic, subTopic);
			assertEquals(pubMessage, subMessage);
			assertTrue(Arrays.equals(pubPayload, subPayload));
			assertEquals(pubQos, subQos);
			//assertEquals(pubRetained, subRetained);	// subscribe のメッセージでは、常に retained=false
		}
	}
}
