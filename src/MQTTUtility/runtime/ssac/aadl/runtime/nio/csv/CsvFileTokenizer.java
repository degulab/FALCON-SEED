...
import ssac.aadl.runtime.nio.TextFileLineTokenizerBase;
...
public class CsvFileTokenizer extends TextFileLineTokenizerBase
{
	...
	/** 読み込み済みレコードのバイト文字列を格納する配列 **/
	private SimpleByteArrayList		_recordBytes = new SimpleByteArrayList(8192);
	/** レコード区切り文字もレコード情報に含めるかどうかのフラグ **/
	private boolean			_storeRecordDelimiter;
	...
	public boolean getRecordBytesStored() {
		return getTokenBytesStored();
	}
	
	public void setRecordBytesStored(boolean toStore) {
		setTokenBytesStored(toStore);
	}
	
	public boolean getRecordDelimiterStored() {
		return _storeRecordDelimiter;
	}
	
	public void setRecordDelimiterStored(boolean toStore) {
		_storeRecordDelimiter = toStore;
	}
	...
	public byte[] getRecordBytes() {
		if (getRecordBytesStored()) {
			return _recordBytes.toArray();
		} else {
			return null;
		}
	}
	...
	public String[] nextRecord() throws IOException
	{
		...
		String recString = csvdecoder.getRecordString();	// レコード区切り文字は含まない
		...
		_recordBytes.clear();
		if (tokenType == TT_EOL) {
			if (getRecordDelimiterStored()) {
				// レコード区切り文字を含める
//				//@@@ test
//				String str = getLastEolString();
//				str = str.replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n");
//				recString += str;
//				//@@@ end of test
				recString += getLastEolString();
				if (getRecordBytesStored()) {
					_recordBytes.append(getTokenBytes());
//					//@@@ test
//					StringBuilder sb = new StringBuilder();
//					byte[] tb = getTokenBytes().get();
//					for (byte b : tb) {
//						if (sb.length() > 0) {
//							sb.append(", ");
//						}
//						sb.append(b);
//					}
//					_recordBytes.append(sb.toString().getBytes(getEncoding()));
//					ByteArray tokenBytes = getTokenBytes();
//					_recordBytes.append(tokenBytes, 0, tokenBytes.size()-getLastEolBytes().size());
//					_recordBytes.append(str.getBytes(getEncoding()));
//					_recordBytes.append(getLastEolBytes());
//					//@@@ end of test
				}
			}
			else {
				// レコード区切り文字を含めない
				//--- recString にはレコード区切り文字は含まれない
				if (getRecordBytesStored()) {
					SimpleByteArrayList tokenBytes = getTokenBytes();
					_recordBytes.append(tokenBytes, 0, tokenBytes.size()-getLastEolBytes().size());
				}
			}
		}
		else if (getRecordBytesStored()) {
			// すべてのレコードバイトを保存
			_recordBytes.append(getTokenBytes());
		}
		clearTokenText();
