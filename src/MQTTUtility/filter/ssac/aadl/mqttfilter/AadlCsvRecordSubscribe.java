...
public class AadlCsvRecordSubscribe extends ssac.aadl.runtime.AADLModule
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	...
	public static void main(String[] args) {
		AadlCsvRecordSubscribe module = new AadlCsvRecordSubscribe();
	...
	/*
	** 指定されたサーバーのトピックに対しサブスクライブし、受信した複数のメッセージをファイルへ保存する。
	** $1[STR]	- 接続先
	** $2[STR]	- トピック
	** $3[STR]	- 伝送品質[QoS] (0 | 1 | 2)
	** $4[STR]	- 受信メッセージ上限数 (0以下は無制限)
	** $5[STR]	- 終了判定タイムアウト[ミリ秒] (0以下は無制限)
	** $6[STR]	- 受信メッセージ終端にレコード区切り文字(改行文字)を付加する (true | false)
	** $7[STR]	- 受信メッセージタイプ (binary | default | platform | 文字コード)
	** $8[OUT]	- 出力先ファイル(CSV)
	*/
	@Override
	protected int aadlRun(String[] args) {
		...
		final String strMsgLimit    = args[3];	// $4[STR]
		final String strTimeout     = args[4];	// $5[STR]
		final String strRecordSep   = args[5];	// $6[STR]
		final String strMsgEncoding = args[6];	// $7[STR]
		final String outFile        = args[7];	// $8[OUT]
		...
		// check message limit
		java.math.BigDecimal msgLimit;
		msgLimit = ssac.aadl.runtime.mqtt.internal.AADLMqttFunctions.convertToDecimal(strMsgLimit, "Invalid the number of message limit");

		// check timeout
		long timeout = ssac.aadl.runtime.mqtt.internal.AADLMqttFunctions.convertToPlusTimeMillis(strTimeout, "Invalid timeout value");
		...
		//@{
			...
			asub.subscribeCsvRecord(strTopic, qos, msgLimit, timeout, appendLineSep, msgEncoding,
