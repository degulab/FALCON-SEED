/*
 * @(#)DtJsonExAlgeSetSerializer.java	0.1.0	2022/07/19
...
/**
 * 交換代数集合(exalge2.ExAlgeSet)の JSON 標準形でのシリアライズ／デシリアライズを行うクラス。<br>
 * JSON 形式は、次の通り。
 * <pre><code>
 * [
 *   <i><b>&lt;交換代数元の JSON 標準形&gt;</b></i>,
 *   ...,
 *   <i><b>&lt;交換代数元の JSON 標準形&gt;</b></i>
 * ]
 * </code></pre>
 * 交換代数集合は、0 個以上の交換代数元(JSON 標準形については {@link DtJsonExalgeSerializer} を参照)を要素とする JSON 配列。
 * 交換代数集合の JSON 標準形では、JSON 配列の要素となる JSON オブジェクトにオブジェクト型名は付加されない。
 * 
 * <p>交換代数集合(exalge2.ExAlgeSet)のオブジェクト型名は、
 * <blockquote>
 * {@code !&ExAlgeSet}
 * </blockquote>
 ...
 */
public class DtJsonExAlgeSetSerializer extends AbDtJsonSerializer
{
	//------------------------------------------------------------
	// Constants
	//------------------------------------------------------------
	
	/** 交換代数集合(exalge2.ExAlgeSet)の JSON 標準形であることを示すタイプ名。 */
	static public final String	TYPENAME	= DtJsonSerializer.OBJTYPENAME_PREFIX_STR + ExAlgeSet.class.getSimpleName();
	 ...
	 * @return	交換代数集合(exalge2.ExAlgeSet)のクラスオブジェクト({@link java.lang.Class}を返す。
	 */
	...
	public void serialize(DtJsonOutputWriter writer, Object target, boolean omitTypeName) throws IOException
	{
		...
		if (target == null) {
			...
			// ExAlgeSet
			ExAlgeSet algeset = (ExAlgeSet)target;
			//--- begin
			writer.beginArray();
			//--- elements
			DtJsonSerializer algeSerializer = DtJsonSerializerManager.instance().getSerializerByClass(Exalge.class);
			for (Exalge elem : algeset) {
				algeSerializer.serialize(writer, elem, OMIT_DATATYPE_NAME);
			}
			//--- end
			writer.endArray();
		}
	...
	public Object deserialize(DtJsonInputReader reader, JSONEventType beginType) throws IOException
	{
		...
		// elements
		DtJsonSerializer algeSerializer = DtJsonSerializerManager.instance().getSerializerByClass(Exalge.class);
		ExAlgeSet newalgeset = new ExAlgeSet();
		etype = reader.nextToken();
		while (etype != null && etype != JSONEventType.END_ARRAY) {
			Exalge newelem = (Exalge)algeSerializer.deserialize(reader, etype);
			if (newelem != null) {
